            ...
            <div class="adv-table">
                    ...
                    <?php foreach($items as $key => $item): ?>
                        ...
                        <div class="form-group<?php echo $class; ?>">
                            ...
                            <?php if($type !== 'hidden'): ?>
                            	<label class="<?php echo $class; ?>" for="<?php echo $key; ?>"><?php echo $texto; ?></label><?php echo (isset($item['help'])) ? ' <span class="badge bg-info'.$help.'"> ? </span>' : NULL; ?><?php echo (isset($item['required'])) ? ' <span class="badge bg-important"> * </span>' : NULL; ?> <?php if(isset($item['button_add']) && $item['button_add'] == TRUE && (!isset($item['readonly']) || $item['readonly'] !== TRUE)): ?><a class="btn btn-info btn-xs tooltips" data-toggle="modal" data-backdrop="static" href="#modal" style="margin-left:10px;" data-title="<?php echo $this->lang->line('agregar_registro_select'); ?>" onclick="javascript:agregar_modal('<?php echo $key; ?>', '<?php echo $item['value']['table']; ?>', '<?php echo $item['value']['key']; ?>', '<?php echo $item['value']['item']; ?>');"><i class="fa fa-plus"></i></a><?php endif; ?>
                            <?php endif; ?>
                            <?php
                            ...
                            <?php if($type === 'google_maps'): ?>
                                <div class="row" style="margin-bottom:10px;">
                                    <div class="col-xs-9">
                                        <input type="text"<?php echo $solo_lectura; ?> class="form-control<?php echo $class; ?>" id="<?php echo $key; ?>" name="<?php echo $key; ?>" value="<?php echo $value; ?>"<?php if(isset($item['placeholder'])): ?> placeholder="<?php echo $item['placeholder']; ?>"<?php endif; ?> />
                                    </div>
